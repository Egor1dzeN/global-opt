...
PYBIND11_MODULE(global_opt_py, m) {
    ...
    py::class_<DEOptions>(m, "DEOptions")
            .def(py::init<>())
            .def_readwrite("popsize", &DEOptions::popsize)
            .def_readwrite("F", &DEOptions::F)
            .def_readwrite("CR", &DEOptions::CR)
            .def_readwrite("random_seed", &DEOptions::random_seed)
            .def_readwrite("maxiter", &DEOptions::maxiter)
            .def_readwrite("tol", &DEOptions::tol);
    // Bind shgo function
    m.def("shgo",
    ...
    m.def("differential_evolution",
          [](py::object func,
             ...
             const DEOptions &options) {
              ...
              return differential_evolution(cpp_func, bounds, options);
          },
          py::arg("func"),
          py::arg("bounds"),
          py::arg("options") = DEOptions(),
          R"(

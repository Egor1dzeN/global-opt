...
TEST(Hill_de_Test, BasicTest_1) {
    int functionId = 1;
    THillProblem problem(functionId);
    double res = calculateTGrishagin_DE(problem);
    EXPECT_NEAR(res, problem.GetOptimumValue(), 0.1);
}

TEST(Hill_de_Test, BasicTest_2) {
    int functionId = 2;
    THillProblem problem(functionId);
    double res = calculateTGrishagin_DE(problem);
    EXPECT_NEAR(res, problem.GetOptimumValue(), 0.1);
...
TEST(Hill_de_Test, BasicTest_100) {
    int functionId = 100;
    THillProblem problem(functionId);
    double res = calculateTGrishagin_DE(problem);
    EXPECT_NEAR(res, problem.GetOptimumValue(), 0.1);
...
TEST(Hill_de_Test, BasicTest_max) {
